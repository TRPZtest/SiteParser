...
namespace UnitTests
{
    public class SiteParserTests
    {
        ...
        public void ExampleTextTests()
        {
            ...
            Assert.Equal(5, results.FirstOrDefault(x => x.Expression == "PDF document")?.Count);

            results = calculator.CalculateFrequencies(text, 1);

            Assert.True(5 == results.Where(x => x.Count == 3).Count()); 
        }
        [Fact]
